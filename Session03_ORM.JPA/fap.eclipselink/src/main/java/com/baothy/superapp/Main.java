...
import com.baothy.superapp.entity.Lecturer;
...
public class Main {
    ...
    private static EntityManagerFactory emf = Persistence.createEntityManagerFactory("com.baothy.superapp.eclipselink-PU");
    ...
    public static void main(String[] args) {
        ...
        // Close the EntityManagerFactory after all operations are complete
        emf.close();
    }
    ...
    public static void update(){
        ...
        Student y = em.find(Student.class, "SE3");
...
//        em.remove(y);
        ...
        em.close(); // Close the EntityManager to prevent resource leaks
    }
    ...
    //EntityManagerlaf ông sếp quản lí các entity ~ hính là các las có @Entity và quản lý các object tạo từ class Entity: sếp có thể thêm persist(); xóa remove();cập nhật merge(); tìm the PK find(): lí do có hàm tìm theo keyvif ta luôn có nhu cầu thao tác rên 1 dòng/row/record cụ thể trong table
    //SWP admin, có màn himhf quản lí user , phân loại , show table có nhiều dòng, phân trang, filter
    public static void findById(){
        EntityManager em = emf.createEntityManager();
        Lecturer x = em.find(Lecturer.class, 1);
        Student y = em.find(Student.class, "SE3");

        System.out.println("Lecturer info: " + x);
    ...
    //CÓ QUYỀN DÙNG TOÁN TỬ LIKE SS GẦN ĐÚNG GIAS TRỊ CHUỖI
    //SQL CHUẨN: SELECT* FROM Lecturer  WHERE Name %AN% --- TÊN CHỨA CHỮ AN
     // %AN% --- TÊN BẮT ĐẦU CHỮ AN
    //PSQL: SELECT lect FROM Name WHERE
    public static void searchLecturers(){
        ...
        //nếu hàm trả về object ta có quyền chấm tiếp thay vì khai bao biến để hứng sau đó biến chấm tiếp
//        String msg = "Hello";
...
//        EntityManagerFactory emf = Persistence.createEntityManagerFactory("com.giaolang.superapp-PU"); //gửi thông số cấu hình Server, nhà thầu JPA: Hibernate, nhà thầu JDBC cho JPA class lo để tạo kết nối tới CSDL cụ thể SQL Server
    ...
    public static void getAllStudents() {
        ...
        em.close();
        // emf.close(); // Removed premature closing of EntityManagerFactory
    }
}
